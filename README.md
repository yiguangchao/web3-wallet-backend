...
JWT 密钥在应用启动时完成校验并只构造一次：`JWT_SECRET` 必须存在且 UTF-8 编码不少于 32 字节，`JWT_EXPIRATION` 必须为正数且生产环境不能超过 24 小时。生产配置不提供密钥默认值，并明确拒绝仓库中公开的开发占位密钥；解析令牌时还要求 `userId` 为正数且用户名不能为空。因此错误配置、弱密钥、超长期令牌或身份声明不完整的令牌都会被提前拒绝。
